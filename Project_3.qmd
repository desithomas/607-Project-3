...
An Entity-Relationship (ER) diagram will be produced to document this design, illustrating how entities relate to one another. We used draw.io to produce this diagram.

**How we will load the data:**

We will load the .csv file into a SQLite database, and use SQL code to generate our structured tables with the appropriate primary keys so that we can link them.

Once the relational database is established, we can use the DBI, RSQLite and tidyverse libraries to perform all data tidying, transformations and exploratory data analysis in R. We will pull only the data that we need for our analysis and address any data quality issues like missing data and outliers that deviate from the standard format.
...
top_skills <- dbGetQuery(con, "
  ...
  CAST(COUNT(j.job_id) AS REAL) / (SELECT COUNT(job_id) FROM jobs) AS pct
  FROM job_skills_bridge b
  JOIN skills_lookup s ON b.skill_id = s.skill_id
  JOIN jobs j ON b.job_id = j.job_id
  WHERE s.skill_name != ''
...
ggplot(top_skills, aes(x = reorder(skill_name, pct), y = pct)) +
  geom_col(fill = "steelblue") +
  # Add percentage text labels at the end of the bars
  geom_text(aes(label = scales::percent(pct, accuracy = 0.1)), 
            hjust = -0.2, size = 3.5, color = "black") +
  coord_flip() +
  # Format the axis to show percentages
  scale_y_continuous(labels = scales::percent_format(), 
                     expand = expansion(mult = c(0, 0.15))) + # Extra room for the text labels
...
*Google DeepMind. (2026). Gemini 3 Flash \[Large language model\]. https://gemini.google.com. Accessed March 16-23, 2026.*
